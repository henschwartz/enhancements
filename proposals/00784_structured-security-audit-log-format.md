...
- Example config + integration test (denial emits audit line)
...
- Integration: `ip_acl` deny + `audit_log` in pipeline → HTTP 403;
  optional assert on captured tracing output for `message=audit`
